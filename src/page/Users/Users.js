...
import {useDebouncedCallback} from "use-debounce"
 ...
 function Users(props) {
    ...
    const onSearch = useDebouncedCallback((value) => {
       setUsers(null);
       history.push({
           search: querryString.stringify({...params, search: value, page: 1})
       })
    },200)
   

    useEffect(() => {
      getFollowsApi(querryString.stringify(params))
      .then(response =>{
    ...
    const onChangeType = type =>{
        ...
        history.push({
            search: querryString.stringify({type: type, page:1, search: ""})
        })
    }
    ...
    const moreData = () => {
        ...
        history.push({
            search: querryString.stringify({...params, page: newPage})
        })
    ...
    return (
        ...
        <Button className={typeUser==="follow" && "active"} 
         onClick={() => onChangeType("follow")}>
            Siguiendo
        </Button > 
        <Button className={typeUser==="new" && "active"}
...
function useUserQuery(location){
    const {page = 1, type = "follow", search=""} = querryString.parse(location.search)
